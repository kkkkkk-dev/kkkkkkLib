...
local Drag = function(Canvas, getMaximizedState, getMinimizedTitleBarState) -- Tambahkan parameter untuk mendapatkan state
    if Canvas then
        local Dragging;
        local DragInput;
        local Start;
        ...
        local function Update(input)
            -- Hanya izinkan dragging jika tidak maximized dan tidak minimized ke title bar
            if not getMaximizedState()() and not getMinimizedTitleBarState()() then -- Panggil fungsi getter
        ...
        Connect(Canvas.InputBegan, function(Input)
            -- Hanya mulai dragging jika tidak maximized dan tidak minimized ke title bar
            if (Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch) and not Type and not getMaximizedState()() and not getMinimizedTitleBarState()() then -- Panggil fungsi getter
                Dragging = true;
    ...
    task.spawn(function()
        ...
        local Resize = function(Delta)
            if Type and MousePos and Size and UIPos and Tab:FindFirstChild("Resize")[Type.Name] == Type then
                ...
                local NewPosition = UDim2.new(UIPos.X.Scale, UIPos.X.Offset + DeltaAnchorOffset.X * Mode.X.X,
...
function Animations:MinimizeToTitleBar(Window: CanvasGroup, Transparency: number)
    local Sidebar = Window:FindFirstChild("Sidebar");
    local Holder = Window:FindFirstChild("Main");
        
    -- Pastikan AnchorPoint untuk posisi top-center saat minimized
    Window.AnchorPoint = Vector2.new(0.5, 0); 
        
    Tween(Window, .25, {
        Size = UDim2.new(0, 200, 0, 30), -- Ukuran contoh bilah judul
        Position = UDim2.new(0.5, 0, 0, 0), -- Posisi contoh (tengah atas)
        GroupTransparency = Transparency or 0,
        Visible = true
    });
    Holder.Visible = false; -- Sembunyikan konten utama
    Sidebar.Size = UDim2.new(1, 0, 1, 0); -- Pastikan sidebar mengisi jendela yang diminimalkan
    Sidebar.Top:FindFirstChild("Title").TextTransparency = 0; -- Pastikan judul terlihat
end

function Animations:RestoreFromMinimizeTitleBar(Window: CanvasGroup, TargetSize: UDim2, TargetPosition: UDim2, TargetTransparency: number)
    local Sidebar = Window:FindFirstChild("Sidebar");
    local Holder = Window:FindFirstChild("Main");
    Holder.Visible = true; -- Tampilkan konten utama
    Window.AnchorPoint = Vector2.new(0.5, 0.5); -- Kembalikan AnchorPoint default
    Tween(Window, .25, {
        Size = TargetSize,
        Position = TargetPosition,
        GroupTransparency = TargetTransparency or 0
    });
end
...
--// Library [Window]
function Library:CreateWindow(Settings: { Title: string, Size: UDim2, Transparency: number, MinimizeKeybind: Enum.KeyCode?, Blurring: boolean, Theme: string, AutoMinimizeOnRun: boolean })
    ...
    local initialWindowSize = Settings.Size or UDim2.new(0, 600, 0, 400); -- Ukuran awal yang diberikan atau default
    local initialWindowPosition = UDim2.new(0.5, 0, 0.5, 0); -- Posisi awal default di tengah layar

    local currentSavedSize = initialWindowSize;    -- Untuk menyimpan ukuran sebelum Maximize/Minimize
    local currentSavedPosition = initialWindowPosition; -- Untuk menyimpan posisi sebelum Maximize/Minimize
        
    -- Pastikan Window memiliki AnchorPoint default 0.5,0.5 agar posisi pusat bekerja
    Window.AnchorPoint = Vector2.new(0.5, 0.5); 
    -- Atur ukuran dan posisi awal jendela
    Window.Size = initialWindowSize;
    Window.Position = initialWindowPosition; 

    local MinimizedTitleBar = false; -- New state for title-bar minimize
    ...
    --// UI Blur & More
    Drag(Window, function() return Maximized end, function() return MinimizedTitleBar end); -- Pass state getters
    Resizeable(Window, Vector2.new(411, 271), Vector2.new(9e9, 9e9));
    Setup.Transparency = Settings.Transparency or 0
    Setup.Size = initialWindowSize -- Gunakan initialWindowSize di sini
            ...
            Connect(Button.MouseButton1Click, function() 
                ...
                elseif Name == "Maximize" then
                    if Maximized then
                        ...
                        Tween(Window, .15, { Size = currentSavedSize, Position = currentSavedPosition });
                        Window.AnchorPoint = Vector2.new(0.5, 0.5); -- Restore default AnchorPoint
                        if BlurEnabled then
                            Blurs[Settings.Title].root.Parent = workspace.CurrentCamera;
                        end
                    else
                        Maximized = true;
                        MinimizedTitleBar = false; -- Pastikan tidak dalam mode title bar minimize
                        Holder.Visible = true; -- Pastikan konten utama terlihat
                                            
                        -- Simpan ukuran dan posisi saat ini sebelum memaksimalkan
                        currentSavedSize = Window.Size;
                        currentSavedPosition = Window.Position;
                                            
                        -- Maksimalkan secara vertikal, pertahankan lebar asli, perluas tinggi hingga penuh
                        -- Atur AnchorPoint ke tengah-atas agar ekspansi vertikal rapi
                        Window.AnchorPoint = Vector2.new(0.5, 0); 
                        
                        Tween(Window, .15, { 
                            Size = UDim2.new(currentSavedSize.X.Scale, currentSavedSize.X.Offset, 1, 0), -- Pertahankan lebar, tinggi penuh
                            Position = UDim2.new(0.5, 0, 0, 0) -- Tengah atas
                        });
                        if BlurEnabled then
                            Blurs[Settings.Title].root.Parent = workspace.CurrentCamera;
                        end
                    end
                    -- Pastikan visibilitas saat memaksimalkan/memulihkan dari maximize
                    if not Opened then
                        Opened = true;
                        Window.Visible = true;
                        Tween(Window, .15, {GroupTransparency = Setup.Transparency});
                    end
                elseif Name == "Minimize" then
                    if MinimizedTitleBar then -- Saat ini dalam mode title bar minimize, kembalikan ke ukuran/posisi sebelumnya
                        MinimizedTitleBar = false;
                        Opened = true;
                        Maximized = false; -- Pastikan tidak dalam mode maximized
                        Animations:RestoreFromMinimizeTitleBar(Window, currentSavedSize, currentSavedPosition, Setup.Transparency);
                        if BlurEnabled then
                            Blurs[Settings.Title].root.Parent = workspace.CurrentCamera;
                        end
                    else -- Saat ini terbuka penuh atau maximized, minimalkan ke title bar
                        MinimizedTitleBar = true;
                        Opened = false;
                        Maximized = false; -- Pastikan tidak dalam mode maximized
                        -- Simpan ukuran dan posisi saat ini sebelum minimize ke title bar
                        currentSavedSize = Window.Size;
                        currentSavedPosition = Window.Position;
                                            
                        Animations:MinimizeToTitleBar(Window, Setup.Transparency);
                        if BlurEnabled then
                            Blurs[Settings.Title].root.Parent = nil;
                        end
                    end
    ...
    SetProperty(Window, { Parent = Screen }); -- Parent terlebih dahulu untuk mendapatkan AbsoluteSize
    if Settings.AutoMinimizeOnRun then
        Animations:MinimizeToTitleBar(Window, Settings.Transparency or 0);
        Opened = false; -- Tandai sebagai tidak terbuka penuh
        MinimizedTitleBar = true;
        Maximized = false; -- Pastikan tidak dalam mode maximized
        if BlurEnabled then
            Blurs[Settings.Title].root.Parent = nil; -- Matikan blur jika diminimalkan
        end
    else
